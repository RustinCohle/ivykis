...
static void __iv_event_run_pending_events(void *_st)
{
	...
	__mutex_lock(&st->event_list_mutex);

	if (iv_list_empty(&st->events_pending)) {
		__mutex_unlock(&st->event_list_mutex);
		return;
	}
...
void iv_event_init(struct iv_state *st)
{
	...
	__mutex_init(&st->event_list_mutex);
...
{
	__mutex_destroy(&st->event_list_mutex);
}
